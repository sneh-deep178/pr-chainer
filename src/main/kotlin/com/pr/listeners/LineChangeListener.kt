    ...
    private fun checkRepositoryChanges() {
        ...
        when (val decision = service.checkIfShouldPrompt()) {
            ...
            is PRChainerService.PromptDecision.Show -> {
                ...
                showMainDialog(decision.totalChanges, decision.threshold, decision.currentBranch, decision.parentBranch)
            }
        }
    }

    private fun showMainDialog(totalChanges: Int, threshold: Int, currentBranch: String, parentBranch: String) {
        dialogs.showMainPrompt(totalChanges, threshold, currentBranch, parentBranch) { result ->
            when (result) {
                ...
                is PRChainerDialogs.MainDialogResult.IncreaseThreshold -> {
                    val newThreshold = service.increaseThreshold(100)
