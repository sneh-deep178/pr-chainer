...
class PRChainerDialogs(private val project: Project, private val service: PRChainerService) {
    ...
    fun showMainPrompt(totalChanges: Int, threshold: Int, currentBranch: String, parentBranch: String, onResult: (MainDialogResult) -> Unit) {
        val branchMessage = if (currentBranch == "master") {
            "⚠️ You're on the 'master' branch!\n\n📊 Working directory: $totalChanges/$threshold lines (${(totalChanges * 100) / threshold}%)\n\nThis will:\n1. Prompt you to enter a branch name\n2. Create a branch with your-name-1\n3. Move your changes to that branch\n4. Create another branch with incremented number\n5. Keep master clean and protected"
        } else {
            val comparisonText = if (parentBranch == "HEAD") "Working directory" else "Branch '$currentBranch' vs '$parentBranch'"
            "📊 $comparisonText: $totalChanges/$threshold lines (${(totalChanges * 100) / threshold}%)\n\nRepository changes exceeded threshold!\n\nCommit current changes and create a new branch?"
        }
        
        val options = arrayOf("Commit & Create Branch", "Increase Threshold (+100)", "Cancel & Increase Threshold")
