...
class PRChainerService(private val project: Project) {
    ...
    private var changeThreshold = 400
    private val originalThreshold = 400
    private var lastCancelTime = 0L
    private val cooldownMs = 10000L // 10 seconds
    ...
    fun checkIfShouldPrompt(): PromptDecision {
        ...
        val parentBranch = gitService.getParentBranch(projectDir, currentBranch)
        ...
        LOG.info("Branch comparison: $currentBranch vs $parentBranch")
        ...
        return when {
            ...
            timeSinceCancel <= cooldownMs -> {
                LOG.info("PRChainerService: Dialog suppressed due to cooldown (${(cooldownMs - timeSinceCancel) / 1000}s remaining)")
                ...
                PromptDecision.Show(totalChanges, changeThreshold, currentBranch, parentBranch)
            }
        }
    }

    fun increaseThreshold(additionalLines: Int = 100): Int {
    ...
    fun handleCancel(): Int {
        ...
        return increaseThreshold(100)
    }

    fun resetThreshold() {
        changeThreshold = originalThreshold
        LOG.info("PRChainerService: Threshold reset to default value: $originalThreshold")
    }
    ...
    sealed class PromptDecision {
        data class Show(val totalChanges: Int, val threshold: Int, val currentBranch: String, val parentBranch: String) : PromptDecision()
